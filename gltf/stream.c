		...
		while (to_read > available) {
			if (!available) {
				...
				if (!gltf_stream->buffer) {
					gltf_stream->buffer_capacity = 8192;
...
static stream_t*
gltf_stream_base64(const char* data, size_t length, size_t unpacked_length) {
	...
	stream->path =
	    string_allocate_format(STRING_CONST("data-base64://0x%" PRIfixPTR), (uintptr_t)stream);
...
gltf_stream_open(gltf_t* gltf, const char* uri, size_t length, unsigned int mode) {
	if ((length > 5) && string_equal(uri, 5, STRING_CONST("data:"))) {
		...
		if (string_equal(STRING_ARGS(encoding), STRING_CONST("base64"))) {
			...
			stream_t* stream = gltf_stream_base64(data.str, data.length, unpacked_length);
			if (stream)
